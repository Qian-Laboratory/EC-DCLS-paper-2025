...
sample_ids <- list.files("/share/home/qlab/project_10x/3_data/project_ec/")
sample_ids
  ...
  tryCatch(
    {
      ...
      scrublet_path <- paste0(
        "/share/home/qlab/project_10x/3_data/project_ec",
...
save(result_list, file = "../result/EC_list.rda")
...
# Merge all samples
ec.merge <- merge(
  x = result_list[1], y = result_list[2:length(result_list)],
  add.cell.ids = sample_id,
  project = "EC"
)
ec.merge <- subset(ec.merge, subset = nFeature_RNA > 200 & nCount_RNA > 400 & percent.mt < 25)
saveRDS(ec.merge, file = "../analysis/EC_merge.rds")
...
# Preprocessing
sample.combined <- readRDS("../analysis/EC_merge.rds")
...
pdf("../figures/umap_integrated_ec_pc30_metadata.pdf", width = 10, height = 8)
...
pdf("../figures/umap_integrated_ec_pc30_feature.pdf", width = 10, height = 10)
...
saveRDS(sample.integrated, "../analysis/sample_integrated_ec_pc30.rds")
